...
| **`CALIBRATE_LLM_PARALLELISM`**    | How many test cases each `calibrate llm` process runs in parallel (`-n`), for both single-agent runs and benchmarks. Default lives in code: `DEFAULT_CALIBRATE_LLM_PARALLELISM` in [`src/utils.py`](src/utils.py) (currently **`4`**).               |
| **`CALIBRATE_SIMULATION_PARALLELISM`** | How many persona/scenario pairs each `calibrate simulations` process runs in parallel (`-n`), for text and voice. Default lives in code: `DEFAULT_CALIBRATE_SIMULATION_PARALLELISM` in [`src/utils.py`](src/utils.py) (currently **`2`**).      |
