...
| Variable                    | Meaning                                                                                                                                                                                                                                                                                       |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`OBJECT_STORAGE_MODE`**   | Artifact storage backend. Defaults to **`s3`** in code/Compose. Set to **`local`** for development without AWS; uploads and job artifacts are stored on disk while keeping `s3://local-dev-artifacts/...` paths in API payloads.                                                               |
| **`LOCAL_ARTIFACT_ROOT`**   | Directory used when `OBJECT_STORAGE_MODE=local`. Defaults to `${DB_ROOT_DIR}/artifacts` in code and `/appdata/artifacts` in Docker Compose. The backend serves these files through development-only `/local-artifacts/...` URLs.                                                              |
| **`LOCAL_ARTIFACT_BASE_URL`** | Optional public backend base URL used when `OBJECT_STORAGE_MODE=local` and the API needs to return artifact download URLs outside a request context. Set to `http://localhost:8000` for normal local dev so frontend audio links point back to the backend.                                   |
