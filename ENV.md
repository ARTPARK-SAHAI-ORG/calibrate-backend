...
| **`PUBLIC_API_BASE_URL`**    | Base URL in **`GET /public-api/openapi.json`** (`servers` block) for Mintlify, public Swagger, and Fern SDK generation. Defaults to **`http://localhost:8000`**; set to the deployment URL in production/staging (deploy workflows export **`secrets.PUBLIC_API_BASE_URL`**). |
