...
| **`DEFAULT_MAX_TRACES`**            | Fallback for the `max_traces` workspace limit, how many traces a workspace may store ([`src/routers/org_limits.py`](src/routers/org_limits.py)). A superadmin can override it per workspace. Enforced by `POST /traces`, which returns 429 once a workspace reaches it. Default **`50000`**. |
| **`DEFAULT_MAX_SCORED_TRACES`**     | Fallback for the `max_scored_traces` workspace limit, how many traces a workspace may have scored ([`src/routers/org_limits.py`](src/routers/org_limits.py)). A superadmin can override it per workspace. Past it a trace is still stored, but its scoring run is written as `skipped` with `over_limit`. Default **`100`**. |
| **`DEFAULT_TRACE_SCORING_BATCH_SIZE`** | Fallback for the `trace_scoring_batch_size` workspace limit, how many of one agent's traces a worker scores in one call to the judging tool ([`src/routers/org_limits.py`](src/routers/org_limits.py)). A superadmin can override it per workspace. Default **`20`**. |
| **`MAX_CONCURRENT_TRACE_SCORING_BATCHES_PER_ORG`** | Fallback for the `max_concurrent_trace_scoring_batches` workspace limit, how many scoring batches one workspace may run at once ([`src/routers/org_limits.py`](src/routers/org_limits.py)). Keeps one workspace's backlog from starving another. Default **`1`**. |
| **`TRACE_SCORING_WORKERS`**         | How many trace-scoring workers the process runs, the global cap on batches in flight ([`src/workers/trace_scoring.py`](src/workers/trace_scoring.py)). Deliberately separate from `MAX_CONCURRENT_JOBS`, which defaults to 1 and would let one long evaluation block every workspace's scoring. Values below 1 are raised to 1. Default **`2`**. |
| **`TRACE_SCORING_WAIT_SECONDS`**    | How many seconds a new trace is held before it is sent to the judging tool ([`src/trace_scoring.py`](src/trace_scoring.py)). Every further trace for the same agent restarts the wait, so it runs from the most recent arrival. The held traces go when the wait passes or the workspace's `trace_scoring_batch_size` is reached, whichever comes first. Set it to `0` to judge every trace the moment it arrives. Default **`120`**. |
| **`TRACE_SCORING_MAX_WAIT_SECONDS`** | The longest a trace may be held, counted from the oldest trace still waiting and never restarted ([`src/trace_scoring.py`](src/trace_scoring.py)). Without it, traces arriving slightly faster than `TRACE_SCORING_WAIT_SECONDS` would restart the wait forever. When it runs out the held traces go, however many there are. Default **`600`**. |
