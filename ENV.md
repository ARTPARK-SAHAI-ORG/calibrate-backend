...
| **`PUBLIC_API_BASE_URL`**    | Base URL in **`GET /public-api/openapi.json`** (`servers` block) for Mintlify, public Swagger, and Fern SDK generation. Default **`https://pense-backend.artpark.ai`**; set **`http://localhost:8000`** for local dev. |
