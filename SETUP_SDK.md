...
**The GCP redeploy is automated (`deploy-mcp` job).** After `publish-mcp`, the `deploy-mcp` job in `publish-sdk.yml` polls npm until `@dalmia/calibrate-mcp@<version>` is served (the npm publish runs asynchronously in the client repo's own `publish.yml`, so we can't dispatch immediately), then `gh workflow run deploy.yml --repo dalmia/calibrate-mcp-deploy -f version=<version>`. That repo builds a container **from the npm package** and redeploys Cloud Run + refreshes the landing page. It lives in `publish-sdk.yml` (not `auto-publish-sdk.yml`) so **any** publish — auto after prod deploy, or a manual `workflow_dispatch` — also redeploys. It skips (no-op) when `SPEAKEASY_API_KEY_MCP` is unset (nothing was published) or if npm hasn't served the version after ~10 min (dispatch `calibrate-mcp-deploy` manually then). **`PUSH_TO_REPO_TOKEN` must have `actions: write` on `dalmia/calibrate-mcp-deploy`** (it already spans the dalmia client repos); the deploy repo authenticates to GCP separately via Workload Identity Federation — see that repo's `README.md`.
