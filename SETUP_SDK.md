...
| **MCP server** (`@dalmia/calibrate-mcp`) | Speakeasy (`mcp-typescript`, 2nd account) | [`dalmia/calibrate-mcp`](https://github.com/dalmia/calibrate-mcp) | `npx @dalmia/calibrate-mcp start` (set `CALIBRATE_API_KEY`) |
  ...
  └─ Auto-publish SDK and CLI (auto-publish-sdk.yml)
       ...
       └─ call publish-sdk.yml
            ...
            └─ publish-mcp (parallel)
                 speakeasy run -t calibrate-mcp (SPEAKEASY_API_KEY_MCP)
...
| `SPEAKEASY_API_KEY` | Speakeasy CLI generate + validate | From [speakeasy.com](https://www.speakeasy.com) (primary account) |
| `SPEAKEASY_API_KEY_MCP` | Speakeasy MCP generate + validate | From a **second** Speakeasy account (standalone MCP / `mcp-typescript` target) |
...
Speakeasy **`mcp-typescript`** generates a standalone MCP server from the same public OpenAPI spec + overlay as the CLI. Uses the **second** Speakeasy account (`SPEAKEASY_API_KEY_MCP`).
...
# Speakeasy CLI + MCP config (MCP uses SPEAKEASY_API_KEY_MCP when set)
...
| MCP generate fails / wrong workspace | `SPEAKEASY_API_KEY_MCP` missing or primary key used | Add second-account key to Production secrets |
