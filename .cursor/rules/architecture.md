...
- **`deploy.yml`** (production, on **release published**) first runs **`detect-code-changes`**, which checks the current release tag against the previous published release. If there is no previous release, tests run. If the diff is empty or contains only `*.md`, `*.yml`/`*.yaml`, or `*.ipynb`, the reusable **`test`** job is skipped and **`build-and-deploy`** proceeds. If any other path changed, **`test`** **`uses: ./.github/workflows/tests.yml`**; a failing test suite prevents the production image build/push and EC2 deploy from running. **`deploy-staging.yml`** does **not** invoke the test workflow; it only runs when manually triggered and goes straight to **`build-and-deploy`**.
