...
| LLM unit test / benchmark | `details.calibrate_config` (frozen calibrate CLI JSON) + `details.evaluators_by_test_name` — per-test name → `[{uuid, calibrate_name, output_type, variable_values, scale_min?, scale_max?}]` | The CLI consumes `calibrate_config`; enrichment uses `evaluators_by_test_name`. Calibrate evaluator name (the `judge_results` dict key) ↔ UUID via the per-test mapping. Different tests can reference different evaluator subsets, so the enrichment snapshot is per-test, not run-level. It also freezes the `variable_values` sent to calibrate and (for rating evaluators) the rubric `scale_min`/`scale_max`, so each `JudgeResult` carries the inputs and bounds the run actually used. | `_enrich_test_results_with_evaluators()` (+ `_enrich_model_results_with_evaluators` for benchmarks) |
...
**Per-run calibrate config (`details.calibrate_config`)** — **why**: Previously the handler called `_build_calibrate_config` to populate `evaluators_by_test_name`, then the worker called it again when the run actually executed. A queued job could wait indefinitely; meanwhile `test_evaluators` links or live evaluator versions could change, so calibrate would emit `judge_results` keyed by names from the **second** build while API enrichment used the **first** snapshot — wrong `evaluator_uuid`s, scales, or fallback paths. Persisting the exact dict passed to calibrate fixes that alignment.

**Pattern**: `run_agent_test` / `run_agent_benchmark` call `_build_calibrate_config(agent, tests)` **once** at submission and store **both** `details.calibrate_config` (full CLI JSON: `params`, `system_prompt`, `tools`, `test_cases`, top-level `evaluators`, etc.) and `details.evaluators_by_test_name`. `run_llm_test_task` and `run_benchmark_task` resolve the CLI payload via `_calibrate_config_from_agent_test_job(task_id, agent, tests)` in `routers/agent_tests.py`, which **`copy.deepcopy`s** stored JSON so worker mutations (benchmarks clearing `params` before `-m`) never alias the persisted row. Queue starters and `job_recovery.py` already pass fresh `agent`/`tests` from the DB into those tasks; they do **not** rebuild config themselves — the worker reads `calibrate_config` from the job row.

**Legacy / fallback**: Jobs created before this field existed, or any row missing `calibrate_config`, fall back to `_build_calibrate_config(agent, tests)` inside `_calibrate_config_from_agent_test_job` (same behavior as the old worker-only rebuild). An empty `{}` is treated as absent and triggers rebuild.
...
**Per-test evaluator snapshot (`details.evaluators_by_test_name`)**: Produced by that same `_build_calibrate_config` call, persisted under `agent_test_jobs.details.evaluators_by_test_name`. Shape:
...
`_build_calibrate_config(agent, tests)` returns **`(config, evaluators_by_test_name)`**. See **Agent Test Job Results Format** (`details.calibrate_config`, `details.evaluators_by_test_name`, `_calibrate_config_from_agent_test_job`, read-time enrichment, and legacy fallback).
...
**Legacy `evaluation.criteria` string** (still in some DB rows; `_build_calibrate_config()` promotes to a synthetic link against the seeded `default-llm-next-reply` evaluator when no `test_evaluators` rows exist). This happens in `_build_calibrate_config()`'s first pass before the final test-case shaping pass. The final response-test handoff then overwrites `evaluation.criteria` with the structured refs from `criteria_per_test`; do not add a later "legacy string" cleanup there, because by that point the field is intentionally no longer the original stored string:
