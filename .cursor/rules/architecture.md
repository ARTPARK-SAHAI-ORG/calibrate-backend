...
| `trace_eval_runs`                 | Durable trace-scoring **runs**. One row per scoring attempt (`pending` / `processing` / `completed` / `failed` / `skipped`); status is the source of truth. At most one **open** (`pending`/`processing`) run per trace (`ux_trace_eval_active`). A rescore or backfill is just another run — no `kind` column. Tables exist; nothing enqueues or scores against them yet.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `trace_eval_scores`                      | Persistent output of a run: one row per `(run_uuid, evaluator_uuid)`. Typed result columns — `match INTEGER` (binary 0/1) XOR `score REAL` (rating), CHECK-enforced — and `evaluator_version_id TEXT NOT NULL`. Same-version rescores are distinct runs and do not overwrite earlier rows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
