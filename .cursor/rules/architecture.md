...
- **Scoring schema + opt-in:** the switch is `config.traces.scoring.enabled` inside `agents.config`, read ONLY through `utils.trace_scoring_enabled` (missing means on, and a value like `0` or `"false"` reads as off). There is no column. Set via `PUT /agents/{uuid}` by sending the stored config with the key; a config body REPLACES the stored config, so the handler carries the key forward when the body omits it, or editing an agent's URL would reset the setting. The off-to-on flip is rejected (422) when zero linked evaluators are eligible; an already-on agent whose evaluators later all become ineligible stays on. Eligibility follows the agent's `interaction_type` (`conversation` -> eval type `response` / `llm`, `general` -> `general` / `llm-general`): matching type, live version present, no declared variables. JWT-only `GET /agents/{uuid}/trace-scoring-eligibility` returns the eligible/ineligible partition. Turning it off deletes that agent's `pending` `trace_eval_runs` and leaves `processing` and terminal runs; the worker ALSO re-reads the setting when it resolves a batch, which closes the race where a trace is ingested in the instant the switch is flipped. **Ingest resolves nothing.** `POST /traces` uses `create_trace_with_eval_run` to insert the trace and, when the switch is on, one run in a single `BEGIN IMMEDIATE` (a deferred `BEGIN` can raise `SQLITE_BUSY` without honouring `busy_timeout`). There is no `scoring_plan`: a run is born `pending`, and the WORKER decides what can score it, once per batch since a batch is one agent (`resolve_batch_evaluators`, asserted). The only skip ingest writes is `over_limit`. **A new run is held, not made claimable at once.** Starting the judging CLI costs the same for one trace or twenty, so a quiet workspace would pay that startup per trace. Inside the same transaction, `_hold_waiting_trace_eval_runs` stamps the arriving run, and moves that agent's still-held runs to match, at the earlier of now + `trace_scoring.WAIT_SECONDS` (`TRACE_SCORING_WAIT_SECONDS`, 120) and the OLDEST waiting run's `created_at` + `MAX_WAIT_SECONDS` (`TRACE_SCORING_MAX_WAIT_SECONDS`, 600), or to now when the agent's waiting count reaches the workspace's `trace_scoring_batch_size` or the wait is 0. Every arrival restarts the first clock; the second never restarts, and exists because arrivals spaced slightly under the wait would otherwise hold the first trace until the batch filled. TWO conditions scope the move and both are load-bearing. `available_at > now` leaves a run that is already claimable alone: a new arrival must not push back a batch only waiting for a free worker, and it keeps the write to the held runs rather than the agent's whole backlog, which would otherwise make ingest slower the further behind scoring fell, inside the one write lock the workers share. `attempts = 0` leaves a run waiting out its retry backoff alone, since moving its time would cut the backoff short or push it further out. No new state: the release time is `available_at`, the column the claim already filters on, so a restart needs no timer. **Ingest therefore wakes a worker only when the runs came out claimable**, not on every trace; a held batch comes due on a time already in the row, which an idle worker sees on its own poll. The FastAPI lifespan starts `workers.trace_scoring` with `TRACE_SCORING_WORKERS` workers (default 2, values below 1 raised to 1), independent of `MAX_CONCURRENT_JOBS_PER_ORG`. **Score reads are JWT-only** (not `Public API`): `GET /traces` rows carry the latest run's `latest_run_status`, `latest_run_error` and its full per-evaluator `results` via one batched `db.get_latest_trace_run_summaries` per page; `GET /traces/{uuid}/scores` returns the run history newest-first (`db.list_trace_scoring_runs`); `GET /traces/usage` reports both counts beside both caps for a banner. `GET /traces?include_score_averages=true` adds `score_averages` beside the page, each evaluator's mean over every matching trace rather than the page, each trace counted once through its latest run whatever its status, the same run a row shows, computed per request rather than stored. Off by default so paging does not pay for a pass over the whole filtered set. Hydration reads the score row's `output_type` and the pinned version's `output_config` (deleted evaluators/versions still render); `passed` follows the CLI rule (`trace_scoring.trace_evaluator_passed`: binary `value == 1`, rating `value == scale_max`). `trace_eval_runs` is one durable run per trace per scoring attempt (`TraceEvalRunStatus`). Indexes: `ux_trace_eval_active` (one open run per trace), `ix_trace_eval_claim` on `available_at` for open rows, `ix_trace_eval_agent_status` `(agent_id, status, completed_at)`, `ix_trace_eval_trace` `(trace_uuid, created_at DESC)`, `ix_trace_eval_org_status` `(org_uuid, status)` for the cap count and the per-workspace in-flight count. `trace_eval_scores` stores one result per `(run_uuid, evaluator_uuid)` with `value` + `output_type` and `evaluator_version_id` NOT NULL, and NOTHING else: it reaches its trace and workspace through its run. A soft-deleted trace's open run is still claimed; settlement is where it becomes `skipped`. Do not key anything on `message_id` / `conversation_id`.
...
- **Cap**: `POST /traces` rejects new rows with **429** (body carries current count, cap, remediation) once the workspace holds `max_traces` live traces, a workspace limit in `org_limits` read by `effective_max_traces`, falling back to `DEFAULT_MAX_TRACES` (50000). A superadmin can raise it for one workspace. The count and the insert are separate statements, so concurrent ingest can overshoot slightly; the cap is a guardrail, not an invariant. A separate `max_scored_traces` (`DEFAULT_MAX_SCORED_TRACES`, 100) caps how many traces a workspace may have SCORED: past it the trace is still stored and its run is written `skipped` with `over_limit`, and a `failed` run frees its slot since the count covers `pending`/`processing`/`completed` only.
...
**Agent test `error` field**: `AgentTestRunListItem` (the runs list) keeps `error` as a `bool`. The detail responses (`TestRunStatusResponse`, `BenchmarkStatusResponse`, `PublicTestRunResponse`, `PublicBenchmarkResponse`) return `error` as text or `null`: the stored `results["error"]` is only a fact the backend actually has, never a sentence written for a reader: calibrate's own `❌`/`✗` line, else its last line with the word `error`, else how the process ended ("calibrate-agent process killed by signal 15 (SIGTERM: Terminated)", "calibrate-agent exited with code 1", "calibrate-agent exited with code 0 but wrote no results.json or metrics.json"), and the generic exception handlers store `f"{type(e).__name__}: {e}"`. The app writes the sentence shown to readers. Paths and tracebacks go to the log and Sentry only. The runs list also carries `stopped_early`.
...
2. Non-zero exit with no `metrics.json` → a crash mid-run (calibrate writes `metrics.json` last). The run is marked failed and the rows `results.json` collected so far, already stored by the intermediate updates, are kept. `no_output_failure` builds a `CliRunFailed`, and the caller logs it, reports it to Sentry and raises it with `error_line` from `cli_error_line`. The handler then runs `_settle_unfinished_results(results, "Failed")`, which marks every row the run never reached `not_run` and sets a still-running model's `message`, the same as `_finish_stopped_run` does with "Stopped".
3. Exit code 0 with neither `results.json` nor `metrics.json` (benchmarks: `_find_all_results_in_output()` empty) → failure, stored as "calibrate-agent exited with code 0 but wrote no results.json or metrics.json".
