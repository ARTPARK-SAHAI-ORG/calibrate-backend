...
**Parameter-name uniqueness**: `POST`/`PUT /tools` reject a `config.parameters` tree where two sibling parameters share a name (case-insensitively, after trimming), recursing into nested object/array parameters (child lists under `parameters`/`properties`/`items`). The walk is scoped to the `parameters` subtree, so a `name` elsewhere in `config` (e.g. a webhook block) never triggers it, and same-named parameters at *different* levels are allowed. Returns **422** from the `ToolCreate`/`ToolUpdate` `model_validator` in `routers/tools.py`. The config is stored opaquely otherwise — this is the one part the backend parses. Backend enforcement of a rule the frontend also applies.
