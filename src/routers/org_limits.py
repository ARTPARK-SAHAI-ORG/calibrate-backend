...
class OrgLimitsCreate(BaseModel):
    org_uuid: str = Field(description="Target organization UUID (8-char identifier)")
    limits: OrgLimits = Field(description="Limit values to set for the org")


class OrgLimitsUpdate(BaseModel):
    limits: OrgLimits = Field(description="New limit values for the org")


class OrgLimitsResponse(BaseModel):
    uuid: str = Field(description="Org-limits row identifier (8-char UUID)")
    org_uuid: str = Field(description="Organization these limits apply to (8-char UUID)")
    limits: OrgLimits = Field(description="Current limit values for the org")
...
async def get_max_rows_per_eval(ctx: OrgContext = Depends(get_current_org)):
    """Get the max rows per eval for the caller's current org. Falls back to
    the server default (`DEFAULT_MAX_ROWS_PER_EVAL`) when no org-specific
    limit is set."""
...
@router.post("", response_model=OrgLimitsCreateResponse, summary="Create org limits")
async def create_org_limits_endpoint(
    data: OrgLimitsCreate, user_id: str = Depends(require_superadmin)
):
    """Create limits for an org. Superadmin only. Returns 404 if the org
...
@router.get("/{target_org_uuid}", response_model=OrgLimitsResponse, summary="Get org limits")
async def get_org_limits_endpoint(
    target_org_uuid: str = Path(description="Target organization UUID (8-char identifier)"),
    ctx: OrgContext = Depends(get_current_org),
):
    """Get limits for an org. The caller must be a member of the target org (or
...
@router.put("/{target_org_uuid}", response_model=OrgLimitsResponse, summary="Update org limits")
async def update_org_limits_endpoint(
    target_org_uuid: str = Path(description="Target organization UUID (8-char identifier)"),
    data: OrgLimitsUpdate = ...,
    user_id: str = Depends(require_superadmin),
):
    """Update limits for an org. Superadmin only. Returns 404 if no limits row
    exists for the org."""
...
@router.delete("/{target_org_uuid}", summary="Delete org limits")
async def delete_org_limits_endpoint(
    target_org_uuid: str = Path(description="Target organization UUID (8-char identifier)"),
    user_id: str = Depends(require_superadmin),
):
    """Delete limits for an org, reverting it to the server default. Superadmin
    only. Returns 404 if no limits row exists for the org."""
