...
def _reset_row(user_uuid: str) -> dict:
    with db.get_db_connection() as conn:
        row = conn.execute(
            "SELECT password_hash, password_reset_token_hash, "
            "password_reset_expires_at FROM users WHERE uuid = ?",
            (user_uuid,),
        ).fetchone()
    return dict(row)
...
def test_password_reset_token_roundtrip():
    email = f"{_u('reset')}@example.com"
    user_uuid = db.create_user_with_password("Re", "Set", email, "old$hash")

    token = db.create_password_reset_token(email)
    assert token

    stored = _reset_row(user_uuid)
    assert stored["password_reset_token_hash"] != token
    assert token not in str(stored)

    assert db.reset_password_with_token(token, "new$hash") is True
    after = _reset_row(user_uuid)
    assert after["password_hash"] == "new$hash"
    assert after["password_reset_token_hash"] is None
    assert after["password_reset_expires_at"] is None

    # A token is single-use.
    assert db.reset_password_with_token(token, "again$hash") is False


def test_password_reset_token_unknown_token_and_email():
    assert db.reset_password_with_token("not-a-real-token", "x$hash") is False
    assert db.create_password_reset_token(f"{_u('nobody')}@example.com") is None


def test_password_reset_token_expired():
    email = f"{_u('expired')}@example.com"
    user_uuid = db.create_user_with_password("Ex", "Pired", email, "old$hash")
    token = db.create_password_reset_token(email)
    ...
    with db.get_db_connection() as conn:
        conn.execute(
            "UPDATE users SET password_reset_expires_at = ? WHERE uuid = ?",
            (past, user_uuid),
        )
        conn.commit()

    assert db.reset_password_with_token(token, "new$hash") is False
    assert _reset_row(user_uuid)["password_hash"] == "old$hash"


def test_password_reset_second_token_invalidates_first():
    email = f"{_u('rereset')}@example.com"
    db.create_user_with_password("Two", "Tokens", email, "old$hash")

    first = db.create_password_reset_token(email)
    second = db.create_password_reset_token(email)
    assert first != second

    assert db.reset_password_with_token(first, "no$hash") is False
    assert db.reset_password_with_token(second, "yes$hash") is True


def test_password_reset_sets_password_for_google_user():
    email = f"{_u('google')}@example.com"
    user_uuid = db.create_user("Goo", "Gle", email)
    assert _reset_row(user_uuid)["password_hash"] is None

    token = db.create_password_reset_token(email.upper())
    assert db.reset_password_with_token(token, "fresh$hash") is True
    assert _reset_row(user_uuid)["password_hash"] == "fresh$hash"


def test_password_reset_token_with_no_expiry_is_refused():
    """A half-written row must read as an invalid link, not crash the request."""
    email = f"{_u('noexpiry')}@example.com"
    user_uuid = db.create_user_with_password("No", "Expiry", email, "old$hash")
    token = db.create_password_reset_token(email)

    with db.get_db_connection() as conn:
        conn.execute(
            "UPDATE users SET password_reset_expires_at = NULL WHERE uuid = ?",
            (user_uuid,),
        )
        conn.commit()

    assert db.password_reset_token_is_valid(token) is False
    assert db.reset_password_with_token(token, "new$hash") is False
    assert _reset_row(user_uuid)["password_hash"] == "old$hash"


def test_password_reset_token_for_a_blank_email_is_refused():
    """The seeded default user can have an empty email; nobody may reset it."""
    assert db.create_password_reset_token("   ") is None
    assert db.create_password_reset_token("") is None
